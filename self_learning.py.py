...
from keras.layers import LSTM, Dense, Bidirectional, Dropout, Conv1D, MaxPooling1D, Flatten, BatchNormalization, TimeDistributed, RepeatVector
...
def load_LIMS_data():
    ...
    data_LIMS['date'] = pd.to_datetime(data_LIMS['date'], format='%d/%m/%Y %H:%M:%S')
    data_LIMS = data_LIMS.fillna(value=0.0)
    labels_LIMS = np.array(data_LIMS.columns[1:])
    labels = []
...
def load_sensors_data(data_date, period_sensors):
    ...
    if data_date.all() == None:
        ...
        for i in h5_list:
            ...
            #start_date = data_loaded.index.min()
            #end_date = data_loaded.index.min() + pd.Timedelta(1095, unit="d")
...
def remove_duplicates(data):
    ...
    for i in data.keys():
        ...
        #data_no_duplicates[i] = data[i].drop_duplicates(['date'], ignore_index=True)
        ...
        for jdx, j in enumerate(unique_dates['date']):
            ...
            if len(np.where(data[i]['date'] == j)[0]) == 2:
                ...
                if np.sum(np.array(data[i].drop(['date', 'time'], axis=1).iloc[np.where(data[i]['date'] == j)[0], :].diff().dropna())) == 0:
                    duplicates_filtered = np.append(
...
def remove_outliers(data, ARIMA_data):
    ...
    for i in data.keys():
        ...
        ARIMA_all = pd.concat([ARIMA_data[i].iloc[outliers.astype('int'), :].drop('date', axis=1).reset_index(drop=True),
...
def prioritisation(data):
    print('Current priority: Amount of data available')
    keys = data.keys()
    ...
    print('Key chosen via prioritisation:', keys_list[np.argmax(data_length)], 'Number of data points: ',
          np.max(data_length))
    ...
    if len(keys) >= 2:
        ...
        for i in keys_sorted:
            ...
            for j in np.delete(keys_sorted, np.where(keys_sorted == i)):
                ...
                results_array = np.append(results_array, ['%s - %s' % (str(i), str(j)), int(models_knn['%s-%s' % (str(i), str(j))].best_params_[
                                          'n_neighbors']), round(mean_absolute_error(y, models_knn['%s-%s' % (str(i), str(j))].predict(X)), 3)])

                """print(i, '/', j,
                      'knn_params = ', model.best_params_['n_neighbors'],
                      'MAE: ', round(mean_absolute_error(y, model.predict(X)), 2),
                      'std: ', round(np.std(y - model.predict(X)), 2))"""
...
def remove_long_term_outliers(data):
    ...
    deviation = f.ppf(p, X_pca.shape[1], X_pca.shape[0]) * (X_pca.shape[0] ** 2- 1) * X_pca.shape[1] / (X_pca.shape[0] * (X_pca.shape[0] - X_pca.shape[1]))
    ...
    #pd.DataFrame(X_pca).to_csv(folder_path + '/PCA.csv', index = False)
...
def feature_pre_selection(data, labels):
    ...
    #results_df.index = data.drop('date', axis = 1).columns
...
def data_split2(X, y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        np.array(X), np.array(y), test_size=0.35, random_state=21)

    scaler = MinMaxScaler()
    scaler = scaler.fit(X_train)

    X_test, X_valid, y_test, y_valid = train_test_split(
        X_test, y_test, test_size=0.3, random_state=21)
    ...
    return X_train, X_test, X_valid, y_train, y_test, y_valid, scaler.transform(X)
...
def time_alignment(priority_key, data_LIMS, data_sensors, description):
    ...
    if np.array(description).all() == None:
        ...
        try:
            ...
            description = description.iloc[np.where(description['Name'] == data_sensors.drop('date', axis = 1).columns)[0], :]
    ...
    for i in range(data_LIMS.shape[0]):
        date_combined.append(str(pd.to_datetime(data_LIMS['date'].iloc[i]).strftime('%d/%m/%Y')) + ' ' + str(pd.to_datetime(data_LIMS['time'].iloc[i]).strftime('%H:%M:%S')))
...
def learning_rate(X_train, X_test, X_valid, y_train, y_test, y_valid, n_hidden_layers, n_nodes, optimiser, loss, activation_function, kernel_initializer, bias_initializer):
    #n_hidden_layers = 2
...
def quick_search(X_train, X_test, X_valid, y_train, y_test, y_valid, labels, folder_models_soft_sensors):
    ...
    print(type(X_train), type(X_test), type(X_valid), type(y_train), type(y_test), type(y_valid))
    ...
    metrics['history'] = optimise_model(X_train, X_test, X_valid, y_train, y_test, y_valid, n_hidden_layers, n_nodes,
                                        optimiser, loss, kernel_initializer, bias_initializer, activation_function, 'quick_search_model', folder_models_soft_sensors, 100, 0)

    end = time.perf_counter()
    metrics['time_elapsed'] = int(end-start)

    model, metrics['training_error'], metrics['testing_error'], metrics['validation_error'], metrics['std'], metrics['max'], metrics['deviations'] = validate_model(
        X_train, X_test, X_valid, y_train, y_test, y_valid, folder_path, optimiser, loss, 'quick_search_model', folder_models_soft_sensors, 'Quick search model')

    feature_importance_dict['feature_imp'], feature_importance_dict['labels'], feature_importance_dict[
        'feature_imp_sorted'], feature_importance_dict['feature_imp_sorted_labels'] = feature_importance(X_train, X_test, X_valid, model, labels)

    if metrics['time_elapsed'] < 60:
        print(f"{metrics['time_elapsed']} seconds")
...
def full_search(X_train, X_test, X_valid, y_train, y_test, y_valid, labels, folder_models_soft_sensors):
    ...
    for i in n_hidden_layers:
        for j in n_nodes:
            ...
            performance_dict['{}_{}'.format(int(i), int(j))]['history'] = optimise_model(X_train, X_test, X_valid, y_train, y_test, y_valid, i, j, optimiser,
                                                                                         loss, kernel_initializer, bias_initializer, activation_function, 'layers_{}_nodes_{}'.format(int(i), int(j)), folder_models_soft_sensors, 100, 0)

            end = time.perf_counter()
            performance_dict['{}_{}'.format(
                int(i), int(j))]['time_elapsed'] = int(end-start)

            model, performance_dict['{}_{}'.format(int(i), int(j))]['training_error'], performance_dict['{}_{}'.format(int(i), int(j))]['testing_error'], performance_dict['{}_{}'.format(int(i), int(j))]['validation_error'], performance_dict['{}_{}'.format(int(i), int(j))]['std'], performance_dict['{}_{}'.format(int(i), int(
                j))]['max'], performance_dict['{}_{}'.format(int(i), int(j))]['deviations'] = validate_model(X_train, X_test, X_valid, y_train, y_test, y_valid, folder_path, optimiser, loss, 'layers_{}_nodes_{}'.format(int(i), int(j)), folder_models_soft_sensors, 'Model architecture: hidden layers - {}, nodes - {}'.format(int(i), int(j)))
            importance_dict['{}_{}'.format(int(i), int(j))]['feature_imp'], importance_dict['{}_{}'.format(int(i), int(j))]['labels'], importance_dict['{}_{}'.format(int(i), int(
                j))]['feature_imp_sorted'], importance_dict['{}_{}'.format(int(i), int(j))]['feature_imp_sorted_labels'] = feature_importance(X_train, X_test, X_valid, model, labels)

    return configuration, performance_dict, importance_dict


def validate_model(X_train, X_test, X_valid, y_train, y_test, y_valid, folder_path, optimiser, loss, model_name, folder_models_soft_sensors, model_type):
...
def optimise_model(X_train, X_test, X_valid, y_train, y_test, y_valid, n_hidden_layers, n_nodes, optimiser, loss, kernel_initializer, bias_initializer, activation_function, model_name, folder_models_soft_sensors, epochs, verbose):
    ...
    history = keras.callbacks.History()
    history = model.fit(X_train, y_train, epochs=epochs, verbose=verbose, batch_size=32, validation_data=(X_test, y_test),
                        shuffle=True, callbacks=[history, model_checkpoint_callback])

    pd.DataFrame(history.history).to_csv(
        folder_models_soft_sensors + '{}.csv'.format(model_name))
...
def LIMS_analyse(folder_results_excel, value_riazi):
    ...
    knn_results_df.round(3).to_excel(
        writer_LIMS, sheet_name ='kNN regression results')
    corr_map.index = corr_map.columns
    corr_map.round(3).to_excel(
        writer_LIMS, sheet_name='LIMS - Correlations map')
    ...
    print('Finding the best key - prioritisation: completed')
...
def sensors_analyse(folder_results_excel, folder_data, data_date, write_sensors_csv_files, period_sensors):
    ...
    feature_preselection_results_df = pd.concat([feature_preselection_results_df,
                                                 pd.DataFrame(labels['Name'].loc[labels['Force'] == 'Yes'].reset_index(drop = True))], axis = 1)
    ...
    data_sensors, data_no_short_outliers, data_smooth, outliers_list, STO_results = remove_short_term_outliers(data_sensors, data_sensors.drop('date', axis = 1).columns)
    ...
    if write_sensors_csv_files == True:
        ...
        print('No short outliers data exported to: {}/data_no_outliers.csv'.format(folder_data))
    ...
    data_sensors.to_csv(folder_path + '/' + 'data_no_outliers.csv', index=False)
    print('Outliers-filtered sensors data exported to: {}/data_no_outliers.csv'.format(folder_path))
    ...
    data_pre_selected, tags_accepted, tags_rejected, results_featurepreselection = feature_pre_selection(data_sensors_no_outliers, labels)
    print('Feature pre-selection with PCA: completed')
    
    if write_sensors_csv_files == True:
        data_pre_selected.to_csv(folder_data + 'data_pre_selected.csv', index = False)
        print('Raw sensors data exported to: {}/data_pre_selected.csv'.format(folder_data))

    feature_preselection_results_df = pd.concat([feature_preselection_results_df,
                                                 pd.DataFrame(tags_accepted, columns = ['Pre-selected features - accepted'])], axis = 1)

    feature_preselection_results_df = pd.concat([feature_preselection_results_df,
                                                 pd.DataFrame(tags_rejected, columns = ['Pre-selected features - rejected'])], axis = 1)

    feature_preselection_results_df = pd.concat([feature_preselection_results_df,
                                                 pd.DataFrame(data_pre_selected.drop('date', axis = 1).columns, columns = ['Final features'])], axis = 1)

    feature_preselection_results_df.columns = ['Original features', 'Forced to accept', 'Pre-selected features - accepted', 'Pre-selected features - rejected', 'Final features']
...
def soft_sensors_analyse(priority_key, data_LIMS, data_sensors, results_holder_dict, value_search, folder_results_excel, folder_models_soft_sensors, description):
    

    writer_soft_sensors = pd.ExcelWriter(folder_results_excel + '/output results - soft sensors.xlsx', engine='xlsxwriter')
    
    data_sensors, data_LIMS, data_date, description = time_alignment(priority_key, data_LIMS, data_sensors, description)
    
    pd.concat([data_date, data_sensors, data_LIMS], axis = 1).to_excel(writer_soft_sensors, sheet_name='Processed & aligned data', index=False)
    ...
    X, y, X_train, X_test, X_valid, y_train, y_test, y_valid, X_scaled, tags, names = data_split(data_sensors, data_LIMS, 0.35, 0.3, 15)
    
    pd.DataFrame(X_train, columns = tags).to_excel(writer_soft_sensors, sheet_name = 'Scaled input X - training', index = False)
    pd.DataFrame(y_train, columns = names).to_excel(writer_soft_sensors, sheet_name = 'Scaled output y - training', index = False)
    pd.DataFrame(X_test, columns = tags).to_excel(writer_soft_sensors, sheet_name = 'Scaled input X - testing', index = False)
    pd.DataFrame(y_test, columns = names).to_excel(writer_soft_sensors, sheet_name = 'Scaled output y - testing', index = False)    
    pd.DataFrame(X_valid, columns = tags).to_excel(writer_soft_sensors, sheet_name = 'Scaled input X - validation', index = False)
    pd.DataFrame(y_valid, columns = names).to_excel(writer_soft_sensors, sheet_name = 'Scaled output y - validation', index = False)    
    ...
    if value_search == False:
        ...
        performance_dict, importance_dict = quick_search(X_train, X_test, X_valid, y_train, y_test, y_valid, tags, folder_models_soft_sensors)
        ...
        if priority_key != None:
            output_df['ARIMA threshold'] = np.round(thresholds)
    ...
    if value_search == True:
        ...
        configuration, performance_dict, importance_dict = full_search(X_train, X_test, X_valid, y_train, y_test, y_valid, tags, folder_models_soft_sensors)
        ...
        for k in range(0, len(names)):
                ...
                for j in configuration['Number of nodes']:
                    ...
                    output_df = np.append(output_df, 
                                      np.array(['Layers: {} x Nodes: {}'.format(int(i), int(j)), 
                                                round(performance_dict['{}_{}'.format(int(i), int(j))]['training_error'][k], 2),
                                                round(performance_dict['{}_{}'.format(int(i), int(j))]['testing_error'][k], 2), 
                                                round(performance_dict['{}_{}'.format(int(i), int(j))]['validation_error'][k], 2), 
                                                thresholds[k]]))
            ...
            output_df.to_excel(writer_soft_sensors, sheet_name='Full search - summary', startrow = 1, startcol = k * 6, index = False)
...
def soft_sensors_visualisation(folder_visualisations, soft_sensors_plot_decision, results_holder_dict, data_holder_dict, value_search, folder_models_soft_sensors):
    ...
    if soft_sensors_plot_decision == True:
        ...
        if bool(value_search) == False:
            ...
            fig_loss_function.add_trace(go.Scatter(x=np.arange(1, len(results_holder_dict['soft_sensors_performance']['history']['loss']) + 1),
                                                   y=results_holder_dict['soft_sensors_performance']['history']['loss'],
                                                   name='Training dataset'))

            fig_loss_function.add_trace(go.Scatter(x=np.arange(1, len(results_holder_dict['soft_sensors_performance']['history']['val_loss']) + 1),
            ...
            time_elapsed = int(np.linspace(1, results_holder_dict['soft_sensors_performance']['time_elapsed'], len(
                results_holder_dict['soft_sensors_performance']['history']['val_loss']))[np.argmin(results_holder_dict['soft_sensors_performance']['history']['val_loss'])])
            ...
            fig_loss_function.add_annotation(x=np.argmin(results_holder_dict['soft_sensors_performance']['history']['val_loss']) + 1,
                                             y=np.min(
            ...
            for idx, i in enumerate(deviations.columns):
                ...
                deviations_histogram.add_trace(go.Scatter(x=data_holder_dict['date_time'],
                                                          ...
                                                          name=i,
                                                          marker_color=px.colors.qualitative.Plotly[idx],
                                                          showlegend=False, mode='markers', text=data_holder_dict['date_time']),
                                               row=3,
                                               col=1)
        ...
        if bool(value_search) == True:
            ...
            def annotation(results_holder_dict, i, j):
                ...
                time_elapsed = int(np.linspace(1, results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['time_elapsed'], len(results_holder_dict['soft_sensors_performance']['{}_{}'.format(
                    int(i), int(j))]['history']['val_loss']))[np.argmin(results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['history']['val_loss'])])
                ...
                annotation = dict(x=np.argmin(results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['history']['val_loss']) + 1,
            ...
            for i in results_holder_dict['soft_sensors_configuration']['Hidden layers']:
                for j in results_holder_dict['soft_sensors_configuration']['Number of nodes']:
                    ...
                    buttons_loss.append(dict(label=f"Hidden layers: {i}, number of nodes: {j}",
                                             method="update",
                                             args=[{"x": [np.arange(1, len(results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['history']['loss']) + 1),
                                                          np.arange(1, len(results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['history']['val_loss']) + 1)],
                                                    'y': [results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['history']['loss'],
                                                          results_holder_dict['soft_sensors_performance']['{}_{}'.format(int(i), int(j))]['history']['val_loss']]},
            ...
            fig_loss_function.add_trace(go.Scatter(x=np.arange(1, len(results_holder_dict['soft_sensors_performance'][list(results_holder_dict['soft_sensors_performance'].keys())[0]]['history']['loss']) + 1),
                                                   y=results_holder_dict['soft_sensors_performance'][list(
                                                       results_holder_dict['soft_sensors_performance'].keys())[0]]['history']['loss'],
                                                   name='Training set'))

            fig_loss_function.add_trace(go.Scatter(x=np.arange(1, len(results_holder_dict['soft_sensors_performance'][list(results_holder_dict['soft_sensors_performance'].keys())[0]]['history']['val_loss']) + 1),
                                                   y=results_holder_dict['soft_sensors_performance'][list(
                                                       results_holder_dict['soft_sensors_performance'].keys())[0]]['history']['val_loss'],
                                                   name='Testing set'))
            ...
            time_elapsed = int(np.linspace(1, results_holder_dict['soft_sensors_performance'][list(results_holder_dict['soft_sensors_feature_importance'].keys())[0]]['time_elapsed'], len(results_holder_dict['soft_sensors_performance'][list(
                results_holder_dict['soft_sensors_feature_importance'].keys())[0]]['history']['val_loss']))[np.argmin(results_holder_dict['soft_sensors_performance'][list(results_holder_dict['soft_sensors_feature_importance'].keys())[0]]['history']['val_loss'])])
            ...
            fig_loss_function.add_annotation(x=np.argmin(results_holder_dict['soft_sensors_performance'][list(results_holder_dict['soft_sensors_performance'].keys())[0]]['history']['val_loss']) + 1,
                                             ...
                                             ax=int(np.argmin(results_holder_dict['soft_sensors_performance'][list(
                                                 results_holder_dict['soft_sensors_performance'].keys())[0]]['history']['val_loss']) * 0.7),
            ...
            # mean
            results_full_search_df_train = pd.DataFrame(np.zeros((int(len(results_holder_dict['soft_sensors_configuration']['Hidden layers']) * len(
                results_holder_dict['soft_sensors_configuration']['Number of nodes'])), len(results_holder_dict['products']))))
            ...
            results_full_search_df_test = pd.DataFrame(np.zeros((int(len(results_holder_dict['soft_sensors_configuration']['Hidden layers']) * len(
                results_holder_dict['soft_sensors_configuration']['Number of nodes'])), len(results_holder_dict['products']))))
            ...
            results_full_search_df_valid = pd.DataFrame(np.zeros((int(len(results_holder_dict['soft_sensors_configuration']['Hidden layers']) * len(
                results_holder_dict['soft_sensors_configuration']['Number of nodes'])), len(results_holder_dict['products']))))
...
def LIMS_visualisation(LIMS_plot_decision, data_holder_dict, results_holder_dict, folder_visualisations):
    ...
    if bool(LIMS_plot_decision) == True:
        ...
        for i in data_holder_dict['data_LIMS_separated_nozeros'].keys():
            ...
            # Histogram
            figure_no_zeros_histogram_with_outliers = ff.create_distplot([data[j] for j in data.drop('date', axis=1).columns[::-1]],
                                                                         data.drop(
                ...
                try:
                    ...
                    for column in data_holder_dict['data_riazi'][i].drop('volume', axis=1).columns.to_list():
                        ...
                        figure_riazi.add_trace(go.Scatter(x=volumes_dict[i].flatten(),
                                                          y=np.array(data_LIMS_separated_nozeros_noduplicates[i].drop(['date', 'time'], axis = 1).iloc[np.where(data_LIMS_separated_nozeros_noduplicates[i]['date'] == column)[0], :]).flatten(),
                                                          name=str(column),
                                                          mode='markers'))
                    ...
                    def create_layout_button(column):
                        print(data_holder_dict['data_riazi'][i].drop('volume', axis=1).columns.isin(
                            [column]))
                        return dict(label=str(column),
                                    method='update',
                                    args=[{'visible': data_holder_dict['data_riazi'][i].drop('volume', axis=1).columns.isin(
                                        [column]),
                                        'title': str(column),
                                        'showlegend': True}])

                    print(list(
                        data_holder_dict['data_riazi'][i].drop(
                    ...
                    for position_volume, volume in enumerate(volumes_dict[i]):
                        ...
                        figure_riazi_deviations.update_layout(title_text="Boxplots representing deviations between actual values of LIMS and those predicted with optimised Riazi model",
                                                              margin=dict(
                                                                  l=1, r=1, b=50, t=50, pad=1),
                    ...
                    figure_riazi_parameters = make_subplots(rows=3,
                                                            ...
                                                            specs=[[{}, {}],
                                                                   ...
                                                                   [{"colspan": 2}, None]],
                                                            subplot_titles=(
                                                                "Parameter A", "Parameter B", "Parameter T0", "Calculated T at 99 vol% (T99)", "Coefficient of determination R2 for actual and predicted values")
                                                            )
                    ...
                    figure_riazi_parameters.add_trace(go.Scatter(x=results_holder_dict['riazi_results_dict'][i]['date'],
                                                                 y=results_holder_dict['riazi_results_dict'][i]['R2_riazi'],
                                                                 name=str(
                                                                     i),
                                                                 mode='markers',
...
def sensors_visualisation(folder_visualisations, sensors_plot_decision, data_holder_dict, results_holder_dict):
    ...
    if bool(sensors_plot_decision) == True:
        ...
        print(folder_visualisations)
        print(folder_visualisations_sensors)
        ...
        for position_j, j in enumerate(data_holder_dict['sensors_no_short_outliers'].drop('date', axis=1).columns):
            ...
            for position_i, i in enumerate(years):
                ...
                buttons.append(dict(label=str(i),
                                    method="update",
                                    args=[{"x": [data_holder_dict['raw_sensors']['date'].iloc[years_id[i]],
                                                 data_holder_dict['sensors_no_short_outliers']['date'].iloc[years_id[i]],
                                                 data_holder_dict['sensors_smooth_data']['date'].iloc[years_id[i]]],
                                           "y": [data_holder_dict['raw_sensors'].iloc[years_id[i], position_j],
                                                 data_holder_dict['sensors_no_short_outliers'].iloc[years_id[i], position_j],
            ...
            sensors_no_short_outliers.add_trace(
                go.Scatter(x=data_holder_dict['sensors_no_short_outliers']['date'].iloc[years_id[list(years_id.keys())[0]]],
            ...
            sensors_no_short_outliers.write_html(folder_visualisations_sensors + '{} - no short term outliers.html'.format(j))
    ...
    for position_i, i in enumerate(years):
        ...
        buttons.append(dict(label=str(i),
                            ...
                            args=[{"x": [data_holder_dict['sensors_no_short_outliers']['date'][years_id[i]],
                                         [np.min(data_holder_dict['sensors_no_short_outliers']['date'][years_id[i]]),
                                          np.max(data_holder_dict['sensors_no_short_outliers']['date'][years_id[i]])]],
    ...
    figure_hotteling.add_trace(
        go.Scatter(x=[np.min(data_holder_dict['sensors_no_short_outliers']['date'][years_id[list(years_id.keys())[0]]]),
        ...
        for position_i, i in enumerate(years):
            ...
            buttons.append(dict(label=str(i),
                                ...
                                args=[{"x": [data_holder_dict['raw_sensors']['date'].iloc[years_id[i]],
                                       ...
                                       "y": [data_holder_dict['raw_sensors'].iloc[years_id[i], position_j],
                                             data_holder_dict['sensors_no_outliers'].iloc[years_id[i], position_j]]}],
                                ))
...
def perform_auto_search(value_riazi, value_search, folder_results, main_decision, write_sensors_csv_files, period_sensors, soft_sensors_plot_decision, LIMS_plot_decision, sensors_plot_decision):
    ...
    if main_decision == 'full':
        ...
        print('Full analysis')
        data_holder_dict_LIMS, results_holder_dict_LIMS = LIMS_analyse(folder_results_excel, value_riazi)
        ...
        data_holder_dict_sensors, results_holder_dict_sensors = sensors_analyse(folder_results_excel, folder_data, data_holder_dict['data_date'], write_sensors_csv_files, None)
        ...
        data_holder_dict_soft_sensors, results_holder_dict_soft_sensors = soft_sensors_analyse(results_holder_dict['priority_key'], data_holder_dict['data_no_outliers'], data_holder_dict['sensors_pre_selected'], results_holder_dict, value_search, folder_results_excel, folder_models_soft_sensors, results_holder_dict['description'])
        ...
        soft_sensors_visualisation(folder_visualisations, soft_sensors_plot_decision, results_holder_dict, data_holder_dict, value_search, folder_models_soft_sensors)
    ...
    if main_decision == 'sensors':
        ...
        data_holder_dict_sensors, results_holder_dict_sensors = sensors_analyse(folder_results_excel, folder_data, np.array([None]), write_sensors_csv_files, period_sensors)
    ...
    if main_decision == 'soft_sensors':
        ...
        data_holder_dict_soft_sensors, results_holder_dict_soft_sensors = soft_sensors_analyse(np.array([None]), np.array([None]), np.array([None]), np.array([None]), value_search, folder_results_excel, folder_models_soft_sensors, [None])
                             ...
                             html.Div([html.Div([html.H2('Step 1: Load the data')], style={'display': 'inline-block'}),
                                       html.Div([dbc.Button('i', id="button_step1", 
                                                            ...
                                                            style = {'padding': '1px 8px',
                                                                    ...
                                                                    'margin-bottom': '60%',
                                                                    'color': '#696969'})], style={'display': 'inline-block'}),
                                                 dbc.Modal([dbc.ModalHeader("Step 1: Load the data"),
                                                            dbc.ModalBody("Information for load the data"),
                                                            dbc.ModalFooter(dbc.Button("Close", id="close_step1", className="ml-auto"))], 
                                                           id="modal_step1")]),
                             ...
                             html.Div([html.H2('Step 2: What do you want to do?', style={'display': 'inline-block'}),
                                      html.Div([dbc.Button('i', id="button_step2", 
                                                 ...
                                                 dbc.Modal([dbc.ModalHeader("Step 2: What do you wanna do?"),
                                                            dbc.ModalBody("Information for what you wanna do?"),
                                                            dbc.ModalFooter(dbc.Button("Close", id="close_step2", className="ml-auto"))], 
                                                           id="modal_step2")]),
                             dcc.RadioItems(id = 'choose_main_option', options = [{'label': 'Full analysis - LIMS, sensors and soft sensors', 'value': 'full'},
...
def main_option(value):
    if value == 'full':
        ...
        return [html.Div([html.H2(title_1), html.Div([html.H2(title_2, style={'display': 'inline-block'}), html.Div([dbc.Button('i', id="button_step3_full", 
                                                            size="sm", 
                                                            style = {'padding': '1px 8px',
                                                                     'border-radius': '100%',
                                                                     'font-size': '12px',
                                                 ...
                                                 dbc.Modal([dbc.ModalHeader("Step 3: Choose other functionalities"),
                                                            dbc.ModalBody("Information for other functionalities"),
    ...
    if value == 'LIMS':
        ...
        content = html.Div([html.H4('Do you want to include Riazi modelling'),
                             ...
                             html.H4('Do you want to write processed sensors data to .csv files?', style = {'display': 'none'}),
                             dcc.RadioItems(id='write_sensors_csv_files',
                                            options=[{'label': 'Yes - write them to .csv files', 'value': 1},
                                                     {'label': 'No - do not write them to .csv files', 'value': 0}],
                                            value=0, labelStyle={'display': 'block'}, style = {'display': 'none'}),
        ...
        return [html.Div([html.H2(title_1), html.Div([html.H2(title_2, style={'display': 'inline-block'}), html.Div([dbc.Button('i', id="button_step3_LIMS", 
                                                            size="sm", 
                                                            style = {'padding': '1px 8px',
                                                                     'border-radius': '100%',
                                                                     'font-size': '12px',
    ...
    if value == 'sensors':
        ...
        return [html.Div([html.H2(title_1), html.Div([html.H2(title_2, style={'display': 'inline-block'}), html.Div([dbc.Button('i', id="button_step3_softsensors", 
                                                            size="sm", 
                                                            style = {'padding': '1px 8px',
                                                                     'border-radius': '100%',
                                                                     'font-size': '12px',
                                                 ...
                                                 dbc.Modal([dbc.ModalHeader("Step 3: Choose other functionalities"),
                                                            dbc.ModalBody("Information for other functionalities"),
                                                            dbc.ModalFooter(dbc.Button("Close", id="close_step3_softsensors", className="ml-auto"))], 
                                                           id="modal_step3_softsensors")]), content], style = {'padding-left': '15px'})]
    ...
    if value == 'soft_sensor':
        ...
        return [html.Div([html.H2(title_1), html.Div([html.H2(title_2, style={'display': 'inline-block'}), html.Div([dbc.Button('i', id="button_step3_sensors", 
                                                            size="sm", 
                                                            style = {'padding': '1px 8px',
                                                                     'border-radius': '100%',
                                                                     'font-size': '12px',
...
def automatic_search(btn1, value_riazi, value_search, LIMS_plot_decision, sensors_plot_decision, soft_sensors_plot_decision, write_sensors_csv_files, main_decision, period_sensors):
    value = [p['value'] for p in dash.callback_context.triggered][0]
    ...
    if value > 0:
        ...
        data_holder_dict, results_holder_dict = perform_auto_search(bool(value_riazi), bool(value_search), folder_results, main_decision, bool(write_sensors_csv_files), int(period_sensors), bool(soft_sensors_plot_decision), bool(LIMS_plot_decision), bool(sensors_plot_decision))
        
        print('Creating plots for LIMS')

        root_folder = folder_path
        folders = ['/figures/LIMS/',
                   '/figures/sensors/raw/',
                   '/figures/sensors/no short term outliers/',
                   '/figures/sensors/no outliers/',
                   '/figures/soft sensors/quick search/',
                   '/figures/soft sensors/full search/']
