...
def prioritisation(data):
    ...
    print('Key chosen via prioritisation:', keys_list[np.argmax(data_length)], 'Number of data points for this property: ',
          np.max(data_length))
...
def feature_pre_selection(data, labels):
    ...
    print('Feature pre-selection. Number of sensors: ' + str(X_std.shape[1]) + ' - Completed')
...
def feature_importance(X_train, X_test, X_valid, model, labels):
    ...
    print('Calculating feature importance for {}'.format(model))
...
def quick_search(X_train, X_test, X_valid, y_train, y_test, y_valid, labels, folder_models_soft_sensors):
    ...
    metrics['history'] = optimise_model(X_train, X_test, X_valid, y_train, y_test, y_valid, n_hidden_layers, n_nodes,
                                        optimiser, loss, kernel_initializer, bias_initializer, activation_function, 'quick_search_model', folder_models_soft_sensors, epochs, 0)
    ...
    for i in n_hidden_layers:
        for j in n_nodes:
            ...
            performance_dict['{}_{}'.format(int(i), int(j))]['history'] = optimise_model(X_train, X_test, X_valid, y_train, y_test, y_valid, i, j, optimiser,
                                                                                         loss, kernel_initializer, bias_initializer, activation_function, 'layers_{}_nodes_{}'.format(int(i), int(j)), folder_models_soft_sensors, epochs, 0)
            ...
            if performance_dict['{}_{}'.format(int(i), int(j))]['time_elapsed'] < 60:
                print(f"It took {performance_dict['{}_{}'.format(int(i), int(j))]['time_elapsed']} seconds to train the network")

            if performance_dict['{}_{}'.format(int(i), int(j))]['time_elapsed'] > 60 and performance_dict['{}_{}'.format(int(i), int(j))]['time_elapsed'] < 3600:
            ...
            if performance_dict['{}_{}'.format(int(i), int(j))]['time_elapsed'] > 3600:
                ...
                minutes = math.floor((performance_dict['{}_{}'.format(int(i), int(j))]['time_elapsed'] - hours * 3600) / 60)
...
def soft_sensors_analyse(priority_key, data_LIMS, data_sensors, results_holder_dict, value_search, folder_results_excel, folder_models_soft_sensors, description):
    ...
    if value_search == False:
        ...
        if priority_key != None:
            output_df['ARIMA threshold'] = float(thresholds)
                             ...
                             html.Div([html.Div([html.H2('Step 1: Load the data')], style={'display': 'inline-block'}),
                                       html.Div([dbc.Button('i', id="button_step1", 
                                                            ...
                                                            style = {'padding': '1px 8px',
                                                                    ...
                                                                    'margin-bottom': '60%',
                                                                    'color': '#696969',
                                                                    'white-space': 'pre-line'})], style={'display': 'inline-block'}),
